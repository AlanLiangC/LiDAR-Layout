...
class LayoutDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,\
        ...
        latent_obj_vecs, obj_embed_ = self.cond_stage_model(enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,\
                                        dec_objs, dec_triples, dec_boxes, dec_angles,\
                                        encoded_dec_text_feat, encoded_dec_rel_feat, dec_objs_to_scene,\
                                        dec_triples_to_scene, missing_nodes, manipulated_nodes)
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None):
        enc_objs, enc_triples  = batch['encoder']['objs'], \
                                batch['encoder']['tripltes'], \
        ...
        return enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat, dec_objs, dec_triples, dec_boxes, dec_angles,\
                encoded_dec_text_feat, encoded_dec_rel_feat, dec_objs_to_scene, dec_triples_to_scene, missing_nodes, manipulated_nodes

    @torch.no_grad()
    def get_input_temp(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None):
        # ground truth
        # import pdb; pdb.set_trace()
        x = super().get_input(batch, k)
    ...
    def shared_step(self, batch, **kwargs):
        ...
        loss = self(enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,\
                    dec_objs, dec_triples, dec_boxes, dec_angles,\
                    encoded_dec_text_feat, encoded_dec_rel_feat, dec_objs_to_scene,\
                    dec_triples_to_scene, missing_nodes, manipulated_nodes)
    ...
    def prepare_df_input(self, triples, obj_embed, relation_cond, scene_ids=None, obj_boxes=None, obj_angles=None):
    ...
    def forward(self, enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,\
        ...
        return self.p_losses(diff_dict, *args, **kwargs)


    def forward_temp(self, x, c, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
            assert c is not None
            if self.cond_stage_trainable:
    ...
    def apply_model(self, x_noisy, t, obj_embed, triples, condition_cross, return_ids=False):

        x_recon = self.model(x = x_noisy, obj_embed=obj_embed, triples=triples, t=t, c_crossattn=condition_cross)
        return x_recon
    ...
    def p_losses(self, box_diff_dict):
        condition_cross = box_diff_dict['c_b']
        obj_embed = box_diff_dict['uc_b']
        data_start = box_diff_dict['box']
        triples = box_diff_dict['preds']
        scene_ids = box_diff_dict['obj_id_to_scene']

        B, D = data_start.shape

        unique_scenes, inv_idx = torch.unique(scene_ids, return_inverse=True)
        t = torch.randint(0, self.num_timesteps, size=unique_scenes.shape,
                          device=self.device).long()  # we want to have different t for each scene not each obj

        t = t[inv_idx]
        assert len(t) == B

        sincos = preprocess_angle2sincos(data_start[:,D-1:D])
        x_start = torch.concat((data_start[:,:D-1],sincos),dim=-1)

        noise = default(None, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, obj_embed, triples, condition_cross)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, **kwargs):
        ...
        elif self.conditioning_key == 'graph_crossattn':
            B, D = x.shape
