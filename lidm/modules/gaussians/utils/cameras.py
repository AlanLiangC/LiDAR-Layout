...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, vfov=None, hfov=None, uid=0,
        ...
        self.projection_matrix = torch.eye(4).to(self.data_device)  # no use
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3]
        self.c2w = self.world_view_transform.transpose(0, 1).inverse()
        self.timestamp = timestamp
        self.grid = kornia.utils.create_meshgrid(self.image_height, self.image_width, normalized_coordinates=False, device=self.data_device)[0]
