...
class Loss(nn.Module):
    ...
    def transform_field(self, field: torch.Tensor):
        ...
        truncation_size = gt_band * self.baseconfig.voxel_size
    ...
    def normal_loss(self, batch, normal_feats: fvnn.VDBTensor, eps=1e-6):
        if self.baseconfig.use_fvdb_loader:
    ...
    def color_loss(self, batch, color_feats: fvnn.VDBTensor):
        assert self.baseconfig.use_fvdb_loader is True
    ...
    def get_kl_weight(self, global_step):
        ...
        if global_step > self.baseconfig.anneal_star_iter:
            if global_step < self.baseconfig.anneal_end_iter:
                kl_weight = self.baseconfig.kl_weight_min + \
                                         (self.baseconfig.kl_weight_max - self.baseconfig.kl_weight_min) * \
                                         (global_step - self.baseconfig.anneal_star_iter) / \
                                         (self.baseconfig.anneal_end_iter - self.baseconfig.anneal_star_iter)
            else:
                kl_weight = self.baseconfig.kl_weight_max
        else:
            kl_weight = self.baseconfig.kl_weight_min
    ...
    def forward(self, batch, out, compute_metric: bool, global_step, current_epoch, optimizer_idx=0):
        ...
        if not self.baseconfig.use_hash_tree:
            gt_grid = out['gt_grid']
            if self.supervision.structure_weight > 0.0:
                for feat_depth, pd_struct_i in out['structure_features'].items():
                    downsample_factor = 2 ** feat_depth
                    if self.baseconfig.remain_h:
        ...
        # compute normal loss
        if self.baseconfig.with_normal_branch:
            ...
            loss_dict.add_loss(f"normal", normal_loss, self.supervision.normal_weight)
        
        # compute semantic loss
        if self.baseconfig.with_semantic_branch:
            for feat_depth, pd_semantic_i in out['semantic_features'].items():
                ...
                if semantic_loss == 0.0: # do not take empty into log
                    continue
                loss_dict.add_loss(f"semantic_{feat_depth}", semantic_loss, self.supervision.semantic_weight)
                
        # compute color loss
        if self.baseconfig.with_color_branch:
            for feat_depth, pd_color_i in out['color_features'].items():
                color_loss = self.color_loss(batch, pd_color_i)
                if color_loss == 0.0:
                    continue
                loss_dict.add_loss(f"color_{feat_depth}", color_loss, self.supervision.color_weight)
        ...
        if "dist_features" in out:
            ...
            for latent_id, (mu, logvar) in enumerate(dist_features):
                ...
                if self.baseconfig.normalize_kld:
                    kld_temp /= num_voxel
            ...
            if self.baseconfig.enable_anneal:
                loss_dict.add_loss("kld", kld, self.get_kl_weight(global_step))
            else:
                loss_dict.add_loss("kld", kld, self.baseconfig.kl_weight)
