...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_positional_embedding:
            self.positional_embedding = nn.Parameter(th.randn(channels // self.num_heads, resolution[0] * resolution[1]) / channels ** 0.5)  # [C,L1]
        else:
...
class ObjectAwareCrossAttention(nn.Module):
    ...
    def forward(self, x, cond_kwargs):
        ...
        if self.norm_first:
            image_patch_positional_embedding = self.norm_for_image_patch_positional_embedding(cond_kwargs['image_patch_bbox_embedding_for_resolution{}'.format(self.resolution[0])])  # (N, encoder_channels, L1)
            ...
            image_patch_positional_embedding = self.layout_position_embedding_projector(
                cond_kwargs['image_patch_bbox_embedding_for_resolution{}'.format(self.resolution[0])]
...
class LayoutDiffusionUNetModel(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_ds:
                    print('encoder attention layer: ds = {}, resolution = {}'.format(ds, self.image_size[0] // ds))
                    for _ in range(self.num_attention_blocks):
                        layers.append(
                            attention_block_fn(
                                ...
                                ds=ds,
                                resolution=[int(sub_size // ds) for sub_size in self.image_size],
                                type='input',
        ...
        print('middle attention layer: ds = {}, resolution = {}'.format(ds, self.image_size[0] // ds))
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                if ds in attention_ds:
                    print('decoder attention layer: ds = {}, resolution = {}'.format(ds, self.image_size[0] // ds))
                    for _ in range(self.num_attention_blocks):
                        layers.append(
                            attention_block_fn(
                                ch,
    ...
    def forward(self, x, timesteps, layout_outputs):
        hs, extra_outputs = [], []
